...
export class UserServiceClient implements IUserServiceClient {
  ...
  constructor() {
    this.axiosInstance = axios.create({
      baseURL: config.services?.userServiceUrl || 'http://localhost:3002',
      timeout: 10000,
      headers: {
        'Content-Type': 'application/json',
        ...(config.services?.userServiceApiKey && {
          'X-API-Key': config.services.userServiceApiKey,
        }),
        ...(config.services?.userServiceInternalToken && {
          'X-Internal-Token': config.services.userServiceInternalToken,
        }),
      },
    });
