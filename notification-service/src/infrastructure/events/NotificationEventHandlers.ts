...
} from '../../domain/repositories/IEventConsumer';
...
export class NotificationEventHandlers {
  ...
  async handleEmailVerificationRequested(event: EmailVerificationRequestedEvent): Promise<void> {
    try {
      ...
      const verificationUrl = `${config.services?.frontendUrl || 'http://localhost:3000'}/verify-email?token=${event.verificationToken}`;
  ...
  async handlePasswordResetRequested(event: PasswordResetRequestedEvent): Promise<void> {
    try {
      ...
      const resetUrl = `${config.services?.frontendUrl || 'http://localhost:3000'}/reset-password?token=${event.resetToken}`;
