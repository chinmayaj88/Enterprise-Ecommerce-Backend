...
export class OCIStreamingEventPublisher implements IEventPublisher {
  ...
  constructor() {
    this.streamId = this.config.events?.ociStreamId || '';
    this.compartmentId = this.config.events?.ociCompartmentId || '';

    if (this.streamId && this.config.events?.ociRegion) {
      try {
        ...
        logger.info('OCI Streaming Event Publisher initialized', {
          ...
          region: this.config.events?.ociRegion,
        });
      } catch (error) {
  ...
  async publish(topic: string, event: Record<string, unknown>): Promise<void> {
    try {
      ...
      const eventPayload = {
        eventType: topic,
        source: (event.source as string) || 'notification-service',
      ...
      logger.debug('Event published successfully to OCI Streaming', {
        topic,
        streamId: this.streamId,
        notificationId: (event as any).notificationId || event.id,
      });
    } catch (error) {
