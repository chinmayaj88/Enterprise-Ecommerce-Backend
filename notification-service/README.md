...
Notification microservice built with **Clean Architecture** principles. This service handles sending notifications via multiple channels (email, SMS, push, in-app), manages email templates, notification preferences, and processes events from other services.
...
- **Infrastructure Layer**: External concerns (database, messaging, cache, logging, config, external service clients, notification providers)
...
- **Email Providers**: SendGrid, Nodemailer
- **SMS Provider**: Twilio
...
EMAIL_PROVIDER=mock  # or sendgrid, nodemailer
...
- **OpenAPI Spec**: `http://localhost:3007/api-docs/openapi.yaml`
...
#### 3. Notification Provider Errors

**Problem**: SendGrid/Twilio/Nodemailer errors

**Solutions**:
```bash
# Verify provider credentials in .env
# Check provider API status
# Use mock provider for testing: EMAIL_PROVIDER=mock
```

#### 4. Port Already in Use

**Problem**: Port 3007 is already in use
