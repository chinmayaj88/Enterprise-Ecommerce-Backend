...
export class PrismaProductRepository implements IProductRepository {
  ...
  async findById(id: string): Promise<Product | null> {
    ...
    if (product) {
      const entity = Product.fromPrisma(product);
  ...
  async findAll(filters?: {
    ...
    return {
      products: products.map((p: any) => Product.fromPrisma(p)),
      total,
    };
  }
  ...
  async getRecommendations(productId: string, limit = 10): Promise<Product[]> {
    ...
    return recommendations.map((p: any) => Product.fromPrisma(p));
  }
  ...
  async getRelatedProducts(productId: string, categoryId?: string, limit = 10): Promise<Product[]> {
    ...
    return related.map((p: any) => Product.fromPrisma(p));
  }
