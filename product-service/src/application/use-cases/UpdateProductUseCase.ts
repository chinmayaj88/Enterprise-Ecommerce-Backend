...
import { ProductUpdatedEvent, ProductPriceChangedEvent, ProductStockChangedEvent } from '../../infrastructure/events/ProductEvents';
  ...
  async execute(id: string, updates: {
    ...
    if ((updates.price !== undefined && updates.price !== existing.price) ||
      ...
      if (this.eventPublisher && updates.price !== undefined && updates.price !== existing.price) {
        ...
        this.eventPublisher.publish('product.price.changed', event).catch((error: any) => {
    ...
    if (this.eventPublisher && updates.stockQuantity !== undefined && updates.stockQuantity !== existing.stockQuantity) {
      const event: ProductStockChangedEvent = {
        ...
        stockStatus: updates.stockStatus || existing.stockStatus,
        timestamp: new Date().toISOString(),
        source: 'product-service',
      };
      this.eventPublisher.publish('product.stock.changed', event).catch((error: any) => {
    ...
    if (this.eventPublisher) {
      ...
      this.eventPublisher.publish('product.updated', event).catch((error: any) => {
