...
[![User Service](https://img.shields.io/badge/Service-User-orange?style=flat-square)](.)

**User profile management, addresses, payment methods, preferences, and wishlist**
...
erDiagram
    ...
    USER_PROFILES ||--o{ RECENTLY_VIEWED_PRODUCTS : "has"
    ...
    USER_PROFILES {
        ...
        int profileCompletionScore "Default: 0, indexed"
        datetime createdAt
        datetime updatedAt
    ...
    PAYMENT_METHODS {
        ...
        string expiryMonth "Nullable"
        string expiryYear "Nullable"
        string cardholderName "Nullable"
        string billingAddressId FK "Nullable, references addresses.id"
        string providerToken "Nullable, encrypted"
        datetime createdAt
        datetime updatedAt
        index userId_type "Composite"
    }
    ...
    WISHLIST_ITEMS {
        string id PK "cuid()"
        string userId FK "References user_profiles.userId, indexed"
        string productId "References product.products.id, indexed"
        string productName "Nullable, denormalized"
        string productImageUrl "Nullable, denormalized"
        string productPrice "Nullable, denormalized"
    ...
    RECENTLY_VIEWED_PRODUCTS {
        ...
        string productName "Nullable, denormalized"
        string productImageUrl "Nullable, denormalized"
        decimal productPrice "Nullable, denormalized"
        datetime viewedAt "Default: now(), indexed"
        unique userId_productId "Composite unique"
    }
    ...
    NOTIFICATION_PREFERENCES {
        string id PK "cuid()"
        string userId FK "References user_profiles.userId, indexed"
        string channel "email|sms|push, indexed"
        string category "orders|promotions|reviews|security|wishlist"
        boolean enabled "Default: true"
        string frequency "Nullable, realtime|daily|weekly|never"
        datetime createdAt
        datetime updatedAt
        unique userId_channel_category "Composite unique"
    }
    
    EVENT_LOGS {
        string id PK "cuid()"
        string eventId UK "Event ID from source, indexed"
        string eventType "Indexed"
        string source "auth-service, etc."
        string payload "JSON string"
        boolean processed "Default: false, indexed"
