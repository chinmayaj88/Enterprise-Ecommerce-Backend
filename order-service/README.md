...
Order management microservice built with **Clean Architecture** principles. This service handles order creation, status management, payment tracking, cancellation, and order notes.
...
docker-compose exec order-service npm run prisma:migrate:deploy
...
#### Order Notes Endpoints (Auth Required)
- `POST /api/v1/orders/:id/notes` - Create order note
- `GET /api/v1/orders/:id/notes` - Get order notes
- `PATCH /api/v1/orders/notes/:id` - Update order note
- `DELETE /api/v1/orders/notes/:id` - Delete order note
