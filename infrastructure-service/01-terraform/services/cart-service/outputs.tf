# ============================================================================
# CART-SERVICE OUTPUTS - Production Grade
# ============================================================================

output "namespace" {
  description = "Kubernetes namespace for cart-service"
  value       = kubernetes_namespace.cart.metadata[0].name
}

output "vault_id" {
  description = "Vault ID for cart-service secrets (shared vault from foundation)"
  value       = var.vault_id
}
...
output "dynamic_group_id" {
  description = "Dynamic group ID for cart-service"
  value       = oci_identity_dynamic_group.cart_service.id
}

output "next_steps" {
  description = "Instructions for deploying cart-service"
  value = <<-EOT
    ✅ Cart-service infrastructure created!
    ...
    3. Deploy cart-service using Kubernetes manifests:
