...
Authentication and authorization microservice built with **Clean Architecture** principles. This service handles user registration, login, JWT token management, password reset, email verification, MFA, device management, session management, and security audit logging.
...
- `DELETE /api/v1/auth/deactivate` - Deactivate account (requires auth)

#### Security Endpoints (Requires Auth)
- `GET /api/v1/security/devices` - Get user devices
- `PUT /api/v1/security/devices/:id` - Update device
...
- `POST /api/v1/security/mfa/verify` - Verify MFA
