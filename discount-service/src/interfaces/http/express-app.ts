...
export function createExpressApp(container: Container): express.Application {
  ...
  const promotionController = container.getPromotionController();
  const discountController = container.getDiscountController();

  // Register routes
  app.use('/api/v1/coupons', createCouponRoutes(couponController));
  app.use('/api/v1/promotions', createPromotionRoutes(promotionController));
  app.use('/api/v1/discounts', createDiscountRoutes(discountController));
  ...
  try {
    ...
    app.use(
      ...
      swaggerUi.setup(openApiSpec, {
        ...
        customSiteTitle: 'Discount Service API Documentation',
