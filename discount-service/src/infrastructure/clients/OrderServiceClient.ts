...
export class OrderServiceClient implements IOrderServiceClient {
  ...
  constructor() {
    const orderServiceUrl = (config as any).orderService?.url || process.env.ORDER_SERVICE_URL || 'http://localhost:3004';
    const orderServiceApiKey = (config as any).orderService?.apiKey || process.env.ORDER_SERVICE_API_KEY;
    const orderServiceInternalToken = (config as any).orderService?.internalToken || process.env.ORDER_SERVICE_INTERNAL_TOKEN;
    ...
    this.axiosInstance = axios.create({
      ...
      headers: {
        ...
        ...(orderServiceApiKey && {
          'X-API-Key': orderServiceApiKey,
        }),
        ...(orderServiceInternalToken && {
          'X-Internal-Token': orderServiceInternalToken,
        }),
      },
    });
