...
export class CartServiceClient implements ICartServiceClient {
  ...
  constructor() {
    const cartServiceUrl = (config as any).cartService?.url || process.env.CART_SERVICE_URL || 'http://localhost:3006';
    const cartServiceApiKey = (config as any).cartService?.apiKey || process.env.CART_SERVICE_API_KEY;
    const cartServiceInternalToken = (config as any).cartService?.internalToken || process.env.CART_SERVICE_INTERNAL_TOKEN;
    ...
    this.axiosInstance = axios.create({
      ...
      headers: {
        ...
        ...(cartServiceApiKey && {
          'X-API-Key': cartServiceApiKey,
        }),
        ...(cartServiceInternalToken && {
          'X-Internal-Token': cartServiceInternalToken,
        }),
      },
    });
