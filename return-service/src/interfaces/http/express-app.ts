...
export function createExpressApp(container: Container): express.Application {
  ...
  app.get('/health', async (_req: express.Request, res: express.Response) => {
    try {
      ...
      res.status(503).json({
        status: 'unhealthy',
        service: 'ecommerce-return-service',
  ...
  // Initialize controller from container
  const returnController = container.getReturnRequestController();

  // API routes
  app.use('/api/v1/returns', createReturnRoutes(returnController));
  ...
  try {
    ...
    app.use(
      ...
      swaggerUi.setup(openApiSpec, {
        ...
        customSiteTitle: 'Return Service API Documentation',
