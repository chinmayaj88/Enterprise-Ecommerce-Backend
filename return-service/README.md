...
Return/Refund microservice built with **Clean Architecture** principles. This service handles return request management, return merchandise authorization (RMA), return status tracking, refund coordination, and return tracking.
...
# Database
DATABASE_URL=postgresql://user:password@localhost:5432/return_db
...
- **Return Service**: `http://localhost:3010`
...
- **OpenAPI Spec**: `http://localhost:3010/api-docs/openapi.yaml`
...
- `POST /api/v1/returns` - Create return request (Auth required)
- `GET /api/v1/returns/:id` - Get return request by ID (Auth required)
- `GET /api/v1/returns/user/:userId` - Get return requests by user (Auth required)
...
**Problem**: Cannot connect to Order/Payment services
