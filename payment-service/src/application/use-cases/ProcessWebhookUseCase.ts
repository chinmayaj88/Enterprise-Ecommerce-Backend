...
import { IEventPublisher, PaymentSucceededEvent, PaymentFailedEvent } from '../../domain/repositories/IEventPublisher';
import { PaymentWebhook, PaymentProvider, WebhookStatus } from '../../domain/entities/PaymentWebhook';
import { PaymentStatus, PaymentProvider as PaymentProviderEnum } from '../../domain/entities/Payment';
import { TransactionStatus } from '../../domain/entities/PaymentTransaction';
  ...
  private getWebhookSecret(provider: PaymentProvider): string {
    switch (provider) {
      case PaymentProvider.STRIPE:
        return config.paymentProviders?.stripe?.webhookSecret || '';
      case PaymentProvider.PAYPAL:
        return config.paymentProviders?.paypal?.webhookId || '';
      default:
        return '';
    }
  }
