...
import { RefundPaymentUseCase } from '../use-cases/RefundPaymentUseCase';
import { GetPaymentUseCase } from '../use-cases/GetPaymentUseCase';
import { CreatePaymentMethodUseCase } from '../use-cases/CreatePaymentMethodUseCase';
import { GetPaymentMethodsUseCase } from '../use-cases/GetPaymentMethodsUseCase';
import { ProcessWebhookUseCase } from '../use-cases/ProcessWebhookUseCase';
...
import { PaymentStatus, PaymentProvider } from '../../domain/entities/Payment';
import { PaymentMethodType } from '../../domain/entities/PaymentMethod';
...
export class PaymentController {
  ...
  async createPayment(req: AuthenticatedRequest & RequestWithId, res: Response): Promise<void> {
    try {
      ...
      sendCreated(res, 201, 'Payment created successfully', payment, req.id);
    } catch (error: unknown) {
  ...
  async processPayment(req: AuthenticatedRequest & RequestWithId, res: Response): Promise<void> {
    try {
      ...
      sendSuccess(res, 200, 'Payment processed successfully', payment, req.id);
    } catch (error: unknown) {
      if (error instanceof AppError) {
        throw error;
      }
  ...
  async refundPayment(req: AuthenticatedRequest & RequestWithId, res: Response): Promise<void> {
    try {
      ...
      sendSuccess(res, 200, 'Payment refunded successfully', refund, req.id);
    } catch (error: unknown) {
      if (error instanceof AppError) {
        throw error;
      }
  ...
  async getPayment(req: AuthenticatedRequest & RequestWithId, res: Response): Promise<void> {
    try {
      ...
      sendSuccess(res, 200, 'Payment retrieved successfully', result, req.id);
    } catch (error: unknown) {
      if (error instanceof AppError) {
        throw error;
      }
  ...
  async getPaymentsByUserId(req: AuthenticatedRequest & RequestWithId, res: Response): Promise<void> {
    try {
      ...
      if (usePagination) {
        ...
        sendSuccess(res, 200, 'Payments retrieved successfully', result, req.id);
      } else {
        ...
        sendSuccess(res, 200, 'Payments retrieved successfully', payments, req.id);
      }
    } catch (error: unknown) {
      if (error instanceof AppError) {
        throw error;
  ...
  async createPaymentMethod(req: AuthenticatedRequest & RequestWithId, res: Response): Promise<void> {
    try {
      ...
      sendCreated(res, 201, 'Payment method created successfully', safePaymentMethod, req.id);
    } catch (error: unknown) {
      if (error instanceof AppError) {
        throw error;
      }
  ...
  async processWebhook(req: RequestWithId, res: Response): Promise<void> {
    try {
      ...
      sendSuccess(res, 200, 'Webhook processed successfully', webhook, req.id);
    } catch (error: unknown) {
      if (error instanceof AppError) {
        throw error;
      }
