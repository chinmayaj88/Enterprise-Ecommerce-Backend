...
export const validateProcessWebhook = [
  body('provider').optional().isString().isIn(['STRIPE', 'PAYPAL', 'MOCK']).withMessage('Invalid provider'),
  body('eventType').optional().isString().notEmpty().withMessage('eventType is required'),
  body('providerEventId').optional().isString().notEmpty().withMessage('providerEventId is required'),
  body('payload').optional().isObject().withMessage('payload is required'),
];
