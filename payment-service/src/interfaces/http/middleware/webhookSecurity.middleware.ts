...
const WEBHOOK_IP_WHITELIST: Record<string, string[]> = {
  stripe: config.paymentProviders?.stripe?.webhookIps || [],
  paypal: config.paymentProviders?.paypal?.webhookIps || [],
};
...
export function verifyWebhookIp(req: Request, res: Response, next: NextFunction): void {
  ...
  if (config.server.env !== 'production' || !process.env.ENABLE_WEBHOOK_IP_WHITELIST) {
    return next();
  }
