...
export class EncryptionService {
   ...
   * In production, use a proper key management service (OCI Vault, HashiCorp Vault)
   */
  private getEncryptionKey(): Buffer {
    // In production, retrieve from key management service
    const encryptionKey = process.env.ENCRYPTION_KEY || config.encryption?.key;
    ...
    if (config.jwt.secret) {
      ...
      return crypto.pbkdf2Sync(config.jwt.secret, 'payment-service-salt', this.iterations, this.keyLength, 'sha256');
    }
