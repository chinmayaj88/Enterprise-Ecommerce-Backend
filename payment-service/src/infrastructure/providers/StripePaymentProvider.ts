...
export class StripePaymentProvider implements IPaymentProvider {
  ...
  constructor() {
    const stripeSecretKey = process.env.STRIPE_SECRET_KEY || config.payment?.stripe?.secretKey;
    if (!stripeSecretKey) {
    ...
    this.webhookSecret = process.env.STRIPE_WEBHOOK_SECRET || config.payment?.stripe?.webhookSecret || '';
