...
import { IPaymentProvider, ChargeRequest, ChargeResponse, RefundRequest, RefundResponse, VerifyWebhookRequest, VerifyWebhookResponse } from '../../domain/repositories/IPaymentProvider';
import { PaymentProvider } from '../../domain/entities/Payment';
...
export class PayPalPaymentProvider implements IPaymentProvider {
  ...
  constructor() {
    const paypalClientId = process.env.PAYPAL_CLIENT_ID || config.payment?.paypal?.clientId;
    const paypalClientSecret = process.env.PAYPAL_CLIENT_SECRET || config.payment?.paypal?.clientSecret;
    ...
    this.webhookId = process.env.PAYPAL_WEBHOOK_ID || config.payment?.paypal?.webhookId || '';
    ...
    try {
      ...
      const orderData = {
        ...
        application_context: {
          ...
          user_action: 'PAY_NOW',
          return_url: `${config.cors?.allowedOrigins?.[0] || 'http://localhost:3000'}/payment/return`,
          cancel_url: `${config.cors?.allowedOrigins?.[0] || 'http://localhost:3000'}/payment/cancel`,
        },
      };
