...
export class ProductServiceClient implements IProductServiceClient {
  ...
  constructor() {
    this.axiosInstance = axios.create({
      baseURL: config.services?.productServiceUrl || 'http://localhost:3003',
      timeout: 10000,
      headers: {
        'Content-Type': 'application/json',
        ...(config.services?.productServiceApiKey && {
          'X-API-Key': config.services.productServiceApiKey,
        }),
        ...(config.services?.productServiceInternalToken && {
          'X-Internal-Token': config.services.productServiceInternalToken,
        }),
      },
    });
