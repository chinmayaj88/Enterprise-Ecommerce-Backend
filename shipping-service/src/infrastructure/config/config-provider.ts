...
interface Config {
  ...
  services?: {
    ...
    productServiceInternalToken?: string;
  };
  events?: {
...
export function getConfig(): Config {
  ...
  mergedConfig = {
    ...
    server: {
      ...mergedConfig.server,
      port: process.env.PORT ? parseInt(process.env.PORT, 10) : mergedConfig.server?.port || 3009,
      env: env,
    ...
    services: {
      orderServiceUrl: process.env.ORDER_SERVICE_URL || mergedConfig.services?.orderServiceUrl || 'http://localhost:3005',
      ...
      productServiceApiKey: process.env.PRODUCT_SERVICE_API_KEY || mergedConfig.services?.productServiceApiKey,
      orderServiceInternalToken: process.env.ORDER_SERVICE_INTERNAL_TOKEN || mergedConfig.services?.orderServiceInternalToken,
      productServiceInternalToken: process.env.PRODUCT_SERVICE_INTERNAL_TOKEN || mergedConfig.services?.productServiceInternalToken,
    },
    events: {
