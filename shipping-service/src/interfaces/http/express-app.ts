...
export function createExpressApp(container: Container): express.Application {
  ...
  const rateController = container.getShippingRateController();
  const methodController = container.getShippingMethodController();
  const zoneController = container.getShippingZoneController();

  // API routes
  app.use('/api/v1/shipments', createShipmentRoutes(shipmentController));
  app.use('/api/v1/shipping/rates', createRateRoutes(rateController));
  app.use('/api/v1/shipping/methods', createMethodRoutes(methodController));
  app.use('/api/v1/shipping/zones', createZoneRoutes(zoneController));

  // API Documentation
  try {
    const baseDir = typeof __dirname !== 'undefined' ? __dirname : process.cwd();
    const openApiSpecPath = path.join(baseDir, '../../../openapi.yaml');
    ...
    app.use(
      ...
      swaggerUi.setup(openApiSpec, {
        ...
        customSiteTitle: 'Shipping Service API Documentation',
